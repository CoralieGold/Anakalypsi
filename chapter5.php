<?php $title = "Le chapitre 5" ?>
<?php include_once("header.php") ?>

<div class="main chapter five">
	<div class="gold-border center">

		<?php include_once("menu.php") ?>

		<div class="row">
			<div class="eleven twelfths skip-one">
				<h1>Chapitre <span class="gold">05</span></h1>
			</div>
		</div>
		<div class="row">
			<div class="eight twelfths skip-two">
				<h2 class="letter-spacing">Ankor</h2>
			</div>
		</div>
		<div class="row">
			<div class="eight twelfths skip-two">
				<p class="white">En continuant votre voyage, vous rencontrez encore une fois un nouveau type de beauté. La beauté mystique. Visitez le temple d’Ankor, profitez toujours du silence.
				</p>
			</div>
		</div>
		<div class="text">
			<div class="row valign-wrapper" style="margin-top:160px;">
				<div class="one half">
					<img src="images/chap5/chap5_img1.jpg" alt="" />
				</div>
				<div class="one half">
					<p class="valign">
						J’aurai presque pensé trop longtemps. Mais j’arrive maintenant dans de nouvelles zones. Ici les structures sont plus précises. Plus belles. Mais la vie continue de l’entourer. Je rentre dans de nouveaux lieux. Je ne sais pas ce qui m’y pousse. Mais je les respecte. La végétation ici est moins présente. Mais autre chose vit ici. Quelque chose qui me dépasse. Qui s’élève. Qui me fait me sentir petit.
					</p>
				</div>
			</div>
			<div class="row">
				<div class="one centered half">
					<p>
						Je reconnais ce sentiment. Je le sentais aussi à ma première prison. Mais cette fois-ci rien ne le contredit. Je n’ai que ce sentiment d’appartenir à quelque chose de plus grand. Je profite de cette présence rassurante.
					</p>
					<p>
						Je ne peux m’empêcher de retourner sur mes pas. Je suis entré dans ce lieu sans y faire attention. Je suis sous un puit de lumière, des ombres légèrement différentes de mes anciens geôliers m’entourent. Celles-ci sont silencieuses. Elle me regardent, leur visage bienveillant me souhaitant la bienvenue. Je reste là, pétrifié. Que faire si elles bougent ? Si elles s’énervent. La lumière s’estompe petit à petit, puis retourne. Les ombres n’ont toujours pas bougé. Je ne suis pas sûr de comprendre.
					</p>
				</div>
			</div>
			<div class="row valign-wrapper">
				<div class="one half">
					<p class="valign">
						Je continue mon retour. J’arrive dans une allée où le ciel me sourit. Devant moi, un arbre géant se tient, droit, le torse bombé. Ses racines s’allongent autour de lui, suit la courbe de la structure. Je ne peux m’empêcher de le comparer à une énorme main, s’assurant que son domicile ne parte pas.
					</p>
					<p>
						Je marche encore. Je retrouve cette matière liquide. Je marche toujours. Je me retourne. Ce sentiment me prend à nouveau. La structure maintenant devant moi s’étend à travers deux cieux. Vers le haut et le bas, dans la terre et l’air. Mais comment puis-je le voir s’étendre sous terre ? J’y vois aussi des nuages, et un soleil. Cette scène est belle, mystérieuse.
					</p>
				</div>
				<div class="one half">
					<img src="images/chap5/chap5_img2.jpg" alt="" />
				</div>
			</div>
			<div class="row">
				<div class="one centered half">
					<p>
						J’ai à nouveau envie de rencontrer ces ombres. Les fondateurs de cet endroit. Je reste persuadé que ce ne sont pas celles que j’ai pu voir à l’intérieur, je n’ai pas vu de vie en elle. Comment peuvent-elles créer autant de lumière ?
					</p>
					<p>
						Je continue mon chemin, suivant la lumière. Je suis maintenant persuadé qu’elles me mèneront aux ombres. Je marche dans un climat plus froid, plus visqueux. Mes pieds ne touchent plus le sol. Je me sens remonter vers la surface de cette matière si fraîche.
					</p>
				</div>
			</div>
		</div>
		<div class="en-bref">
			<div class="row">
				<div class="eight twelfths skip-two">
					<h2 class="letter-spacing white bold">En Bref</h2>
				</div>
			</div>
			<div class="row">
				<div class="eight twelfths skip-two">
					<p class="white">
						Ankor est une cité fantôme, ancienne capitale de l’empire Khmer, devenue florissante entre le 9e et le 15e siècle. Celle-ci fait maintenant office de temple buddhiste. Ses sculptures majestueuses ainsi que son aspect hors du temps ne saura que vous ravir.<br>
						Ankor est situé au Cambodge, au nord du lac Tonlé Sap et au sud de la chaine de montagne Phnom Kulen.
					</p>
				</div>
			</div>
		</div>
		<div class="map-chapter">
			<img src="images/chap5/chap5_map.jpg" alt="" />
		</div>
		<div class="next-chapter">
			<div class="row">
				<div class="one half">
					<a href="chapter4.php" class="valign-wrapper">
						<div class="lineLeft valign"></div>
						<div class="dotLeft valign"></div>
						<p class="linkLeft valign white uppercase letter-spacing semibold">Chapitre<span class="gold">04</span></p>
					</a>
				</div>

				<div class="one half">
					<a href="chapter6.php" class="valign-wrapper">
						<p class="linkRight valign white letter-spacing semibold"><span class="uppercase">Chapitre</span><span class="gold">06</span><br>Ankor</p>
						<img class="dotRight valign"src="images/chap6/chap6_small.jpg" alt="Aller au chapitre 6"/>
						<div class="lineRight valign"></div>
					</a>
				</div>
			</div>
		</div>
	</div>
</div>

<?php include_once("footer.php") ?>
